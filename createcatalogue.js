...
// 文件名在下面数组中项的都排除掉
const excludeFile = ['img', 'LICENSE', 'README.md', 'shell.sh', path.basename(__filename)]
...
;(function formatLink(obj = structs, basePath = '', level = 1) {
  Object.keys(obj).forEach(k => {
    if (k === '_children') {
      // 这个是针对根目录下存在的独立文件
      return readmeContent += obj[k].reduce((t, c) => t + `- [${c.split('.')[0]}](/${c})\n`, '')
    }
    readmeContent += ('\t'.repeat(level - 1) + `- [${k}](${basePath}/${k})\n`)
    // 如果存在子层级，则遍历子层级
    if (obj[k]._children) {
      readmeContent += obj[k]._children.reduce((t, c) => {
        return t + '\t'.repeat(level) + `- [${c.split('.')[0]}](${basePath}/${k}/${c})\n`
      }, '')
    }
